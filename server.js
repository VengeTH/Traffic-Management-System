...
const limiter = rateLimit({
  ...
  max: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) || 100, // 100 per 15 min
  message: {
...
// * NOTE: CSRF middleware is currently disabled inside the middleware itself
...
const validateEnvironment = () => {
  ...
  if (isProduction) {
    ...
    const hasPayMongo =
      ...
      process.env.PAYMONGO_SECRET_KEY.startsWith("sk_")
    const hasGCash =
      process.env.GCASH_CLIENT_ID && process.env.GCASH_CLIENT_SECRET
    const hasMaya = process.env.MAYA_CLIENT_ID && process.env.MAYA_CLIENT_SECRET
    const hasDragonPay =
      process.env.DRAGONPAY_MERCHANT_ID && process.env.DRAGONPAY_PASSWORD

    if (!hasPayMongo && !hasGCash && !hasMaya && !hasDragonPay) {
      logger.warn(
        "⚠️  No payment gateway configured in production. Payment features will not work."
      )
    }
  }
  ...
  if (invalid.length > 0) {
    ...
    invalid.forEach(({ name, currentLength, requiredLength, description }) => {
      logger.error(
        `  - ${name}: ${description} must be at least ${requiredLength} characters (current: ${currentLength})`
      )
    })
    throw new Error(
      `Invalid environment variables: ${invalid.map((v) => v.name).join(", ")}`
    )
  }
