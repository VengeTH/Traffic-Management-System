  ...
  if (!violation) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 rounded-3xl border border-gray-200 bg-gray-50 px-6 py-12 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900">Violation not found</h2>
        <p className="text-sm text-gray-600">Return to the search page and select a valid record.</p>
  ...
  return (
    <div className="space-y-8 px-4 pb-12 pt-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <PageSection title="Incident snapshot" description="Time, location, and contextual information">
          <div className="grid gap-4 md:grid-cols-2">
            <DetailTile label="OVR" value={violation.ovrNumber} />
            <DetailTile label="Citation" value={violation.citationNumber} />
            <DetailTile label="Violation" value={formatViolationLabel(violation.violationType)} />
            <DetailTile label="Vehicle" value={`${violation.vehicleType.toUpperCase()} · ${violation.plateNumber}`} icon={<Car className="h-4 w-4 text-primary-500" />} />
            <DetailTile label="Date" value={new Date(violation.violationDate).toLocaleDateString()} icon={<Calendar className="h-4 w-4 text-primary-500" />} />
            <DetailTile label="Time" value={violation.violationTime} />
          </div>

          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <LocationTile location={violation.violationLocation} description={violation.violationDescription} />
            <OfficerTile enforcer={violation.enforcerName} status={violation.status} />
          </div>
        </PageSection>

        <PageSection title="Financial summary" description="Outstanding amounts and payment history">
          <div className="rounded-3xl border border-gray-100 bg-white/95 p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-500">Base fine</span>
              <span className="text-sm font-bold text-gray-900">₱{violation.baseFine.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
            </div>
            {violation.additionalPenalties > 0 && (
              <div className="mt-3 flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-500">Penalties</span>
                <span className="text-sm font-bold text-warning-700">₱{violation.additionalPenalties.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
              </div>
            )}
            <div className="mt-5 rounded-2xl bg-primary-50/60 p-4">
              <div className="flex items-center justify-between text-lg font-black text-primary-700">
                ...
                <span>₱{violation.totalFine.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-3">
              ...
              <Button variant="outline" className="w-full" onClick={() => {/* TODO: open dispute */}}>
...
const LocationTile: React.FC<LocationTileProps> = ({ location, description }) => (
...
const OfficerTile: React.FC<OfficerTileProps> = ({ enforcer, status }) => (
  <div className="rounded-3xl border border-primary-100 bg-primary-50/60 p-5 shadow-sm">
    ...
    <p className="mt-2 text-xs text-gray-600">Current status: {status.toUpperCase()}</p>
  </div>
);
