...
const MyIssuedViolationsPage: React.FC = () => {
  ...
  const fetchViolations = useCallback(async () => {
    try {
      ...
      const response = await apiService.getEnforcerViolations(params);
      const payload = unwrapApiResponse<
        { violations?: Violation[]; pagination?: { pages?: number; total?: number } } | { data?: Violation[] } | Violation[]
      >(response);

      const violationList: Violation[] = Array.isArray(payload)
        ? payload
        : (payload as { violations?: Violation[] })?.violations ??
          ((payload as { data?: Violation[] })?.data ?? []);

      const paginationData = !Array.isArray(payload)
        ? (payload as { pagination?: { pages?: number; total?: number } })?.pagination
        : undefined;
  ...
  return (
    <div className="space-y-6 sm:space-y-8 px-2 sm:px-4 pb-8 sm:pb-12 pt-6 sm:pt-8 w-full max-w-full overflow-x-hidden">
      <PageHeader
        title="My Issued Violations"
        subtitle="View and manage all violations you have issued."
        icon={FileText}
        actions={
          <Button variant="primary" onClick={() => navigate("/enforcer/violations")}>
      ...
      <PageSection title="Issued violations" description={`Displaying ${violations.length} of ${total} violations`}>
        {loading ? (
          ...
          <Card className="border border-dashed border-gray-300 bg-gray-50 py-12 text-center">
            <CardContent>
              ...
              <h3 className="mt-4 text-lg font-semibold text-gray-900">No violations issued yet</h3>
              <p className="mt-2 text-sm text-gray-600">Start by issuing your first violation citation.</p>
              <div className="mt-6">
                <Button variant="primary" onClick={() => navigate("/enforcer/violations")}>
                  Issue New Violation
                </Button>
                ...
                <table className="min-w-full divide-y divide-gray-200 w-full">
                  <thead className="bg-gradient-to-r from-primary-50 to-white">
                    <tr>
                      ...
                      <th className="px-4 xl:px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Driver</th>
                      ...
                      <tr key={violation.id} className="transition-colors duration-150 hover:bg-primary-50/50">
                        ...
                        <td className="px-4 xl:px-6 py-4 text-right text-sm">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button variant="primary" size="sm" onClick={() => navigate(`/violations/${violation.id}`)}>
                              Details
                            </Button>
                          </div>
                        </td>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100">
                        <div>
                          ...
                          <p className="text-sm font-semibold text-gray-900 mt-0.5 truncate">{violation.driverName}</p>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-0.5 truncate">{violation.violationLocation}</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100">
                        <div>
                      ...
                      <div className="flex gap-2 pt-3 border-t border-gray-100">
                        ...
                        <Button variant="primary" size="sm" className="flex-1" onClick={() => navigate(`/violations/${violation.id}`)}>
                          Details
                        </Button>
                      </div>
                    </div>
...
export default MyIssuedViolationsPage;
