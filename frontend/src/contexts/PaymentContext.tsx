...
interface PaymentContextType extends PaymentState {
  ...
  confirmPayment: (paymentId: string, gatewayTransactionId: string) => Promise<Payment>;
...
export const PaymentProvider: React.FC<PaymentProviderProps> = ({ children }) => {
  ...
  // Initiate payment
  const initiatePayment = async (paymentData: PaymentInitiationRequest): Promise<Payment> => {
    try {
  ...
  // Confirm payment
  const confirmPayment = async (paymentId: string, gatewayTransactionId: string): Promise<Payment> => {
    try {
      dispatch({ type: 'PAYMENT_START' });
  ...
  const calculateProcessingFee = (amount: number, gatewayId: string): number => {
